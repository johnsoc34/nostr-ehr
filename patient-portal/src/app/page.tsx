      ...
      try {
        ...
        if (plain) {
          ...
          if (guardianPkHex) {
            ...
            const isDoctorToChild = ev.pubkey === practicePk && !guardianOfTag;
            if (!isChildGuardianMsg && !isReplyInChildThread && !isDoctorToChild) { return; }
...
function AppointmentsView({ keys, calendarApi, T, onJoinVideo }: { keys: PatientKeys | null; calendarApi?: string; T: Theme; onJoinVideo?: (appointmentId: number) => void }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // Load appointments
    fetch(`${calendarApi}/api/appointments/patient/${encodeURIComponent(npub)}`).then(r => r.ok ? r.json() : []).then(data => {
